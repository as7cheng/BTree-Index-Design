 ...
 * 
 * Description: main file to help test all implementations
 * 
 * @section LICENSE
...
// If the relation size is changed then the second parameter 2 chechPassFail may need to be changed 
// to number of record that are expected to be found during the scan, else tests will erroneously be 
...
void errorTests();
void deleteRelation();
void createRelationSizedRandom(int size);
void createRelationRange(int initial, int finish);
void intEmptyTest();
void int1leafTest();
void intNegTest();
void indexArgTest(int arg);
void test4();
void test5();
void test6();
...
/**
 * @brief Additional test for testing the empty tree
 * 
 */
void test4()
{
	std::cout << "---------------------" << std::endl;
	std::cout << "The additional test4 for an empty tree" << std::endl;
	// Create a tree which is empty
	createRelationSizedRandom(0);
	indexArgTest(0);
	deleteRelation();
}

/**
 * @brief Additional test for testing the tree only have one leaf and it is also the root
 * 
 */
void test5()
{
	std::cout << "---------------------" << std::endl;
	std::cout << "The additional test5 for only one leaf which is the root" << std::endl;
	createRelationSizedRandom(600);
	indexArgTest(6);
	deleteRelation();
}

/**
 * @brief Additional test for testing if the tree has negative keys to insert
 * 
 */
void test6()
{
	std::cout << "---------------------" << std::endl;
	std::cout << "The additional test5 for keys have negative values" << std::endl;
	createRelationRange(-1500, 1500);
	indexArgTest(5);
	deleteRelation(); 
}
...
/**
 * @brief Create a Relation with random size, which is similar to  createRelationRandom()
 * 
...
/**
 * @brief Create a Relation with a range, which is similar to createRelationForward()
 * 
 * @param initial the range starts with
 * @param finish the range ends with
 */
void createRelationRange(int initial, int finish)
{
	std::vector<RecordId> ridVec;
...
/**
 * @brief Additional test for testing tree with 0 records, which is similar to intTests()
 * 
 */
void intEmptyTest()
{
...
/**
 * @brief Additional test for testing if the tree only have one leaf node
 * 
 */
void int1leafTest()
{
...
/**
 * @brief Additional test for testing when we have negative values
 * 
 */
void intNegTest()
{
...
/**
 * @brief Additional test for testing whn we check index whith diffrent arguments to test 
 * 		  one leaf, negative values or empty tree
 * 
 * @param arg number of inputs
 */
void indexArgTest(int arg)
{
	if(testNum == 1)
