...
class BTreeIndex {
 ...
 private:
  ...
  /**
   * The pageId when the root hasn't been split 
   */
  ...
  */
   ...
   * 
   */
  const void insertToLeafNode(LeafNodeInt *node, RIDKeyPair<int> entry);

  /**
   * Create a new root, insert the new entry and update the new header page
   * @param firstOid            The pageId of the first pointer in the root page
   * @param newEntry            The keyPair that is pushed up after splitting
  */
  const void moveKeyUp(PageId firstPid, PageKeyPair<int> *newEntry);
   ...
   * @return false 
   */
  const bool checkKeyInRange(int lowVal, const Operator lowOp, int highVal, const Operator highOp, int key);
  ...
  /**
    ...
    * @return int 
  */
  int findNonLeafLen(NonLeafNodeInt *node);
   ...
   * @return int 
  */
  int findLeafLen(LeafNodeInt *node);
   ...
   * @return int 
  */
  int findArrayIndex(NonLeafNodeInt *node, int index, int key);
   ...
   * @param newEntry            The entry needed to be inserted
   */
  const void insertToNonLeafNode(NonLeafNodeInt *node, PageKeyPair<int> *newEntry);
   ...
   */
  const void findNextNonLeafNode(NonLeafNodeInt *node, PageId &nextNodeNum, int val);
